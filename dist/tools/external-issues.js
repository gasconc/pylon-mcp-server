...
const tools = [
    {
        ...
        description: 'Link or unlink external issues (from Linear, Asana, Jira, or GitHub) to/from a Pylon issue.',
        inputSchema: {
            type: 'object',
            properties: {
...
async function handleToolCall(name, args) {
    switch (name) {
        case 'pylon_link_external_issue': {
            ...
            const result = await linkExternalIssue(issue_id, external_issue_id, source, operation);
...
const externalIssuesModule = { tools, handleToolCall };
