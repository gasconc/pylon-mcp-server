    ...
    inputSchema: {
      ...
      properties: {
        filter: {
          type: 'object',
          description:
            'Filter object for searching. Use "and"/"or" operators with subfilters for complex queries.',
          properties: {
...
async function handleToolCall(name: string, args: Record<string, unknown>): Promise<ToolResult | null> {
  switch (name) {
    ...
    case 'pylon_search_issues': {
      ...
      const result = await searchIssues(filter as any ?? null, (cursor as string) ?? null, limit);
      return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
    }
    default:
      return null;
...
const issuesModule: ToolModule = { tools, handleToolCall };
